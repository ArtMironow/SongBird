...
class SongBird {
    ...
    init(){
      var el_1 = document.getElementById('el_1');
      let bird = birdsData[songBird.stage];
      el_1.innerText = (bird.find(obj => obj.id === 1)).name;
      var el_2 = document.getElementById('el_2');
      el_2.innerText = (bird.find(obj => obj.id === 2)).name;
      var el_3 = document.getElementById('el_3');
      el_3.innerText = (bird.find(obj => obj.id === 3)).name;
      var el_4 = document.getElementById('el_4');
      el_4.innerText = (bird.find(obj => obj.id === 4)).name;
      var el_5 = document.getElementById('el_5');
      el_5.innerText = (bird.find(obj => obj.id === 5)).name;
      var el_6 = document.getElementById('el_6'); 
      el_6.innerText = (bird.find(obj => obj.id === 6)).name;

      var score = document.getElementById('score');
      score.innerText = 'Score: ' + this.score.toString();

      this.setHeaderColor();
    }
    
    setAudio(pos, id, id_source){
      ...
      if(pos === 0){
        ...
        let bird = birdsData[songBird.stage];
        source.src = (bird.find(obj => obj.id === pos)).audio;
        audio.load(); 
      }
    }
    ...
    checkResult(pos){
      if(pos === this.rand){
        for (let index = 0; index < this.isClicked.length; index++) {
          this.score += this.isClicked[index];         
        }

        this.setResult();
        this.freeze = true;

        this.setListColor(pos, 'color-green');
      }else{
        //console.log('No');
        this.setListColor(pos, 'color-red');
      }
    }
    ...
    setResult(){
      ...
      var score = document.getElementById('score');
      var picture = document.getElementById('bird-picture');
      let bird = birdsData[songBird.stage];
      picture.src = (bird.find(obj => obj.id === this.rand)).image;

      var name = document.getElementById('bird-name');
      name.innerText = (bird.find(obj => obj.id === this.rand)).name;
    ...
    displayChange(){
        ...
        var emptyDiv = document.querySelector("div.center-bottom-right-empty");
        emptyDiv.classList.add("no-display");
        var div = document.querySelector("div.center-bottom-right");
    ...
    getBeep(pos){     
      if (songBird.rand === pos){
        var audio = new Audio("../../Assets/sound/true.mp3");
        audio.load(); 
        audio.play(); 
      }else{
        var audio = new Audio("../../Assets/sound/false.mp3");
        audio.load(); 
        audio.play(); 
      }
    }
    ...
    setHeaderColor(){
      if(this.stage === 0){
        var top = document.querySelector("li.top-1");
        top.classList.add("color-green");
      }else{
        var currentStage = this.stage;
        var top = document.querySelector("li.top-" + currentStage.toString());
        top.classList.remove("color-green");
        var topNext = document.querySelector("li.top-" + (currentStage + 1).toString());
        topNext.classList.add("color-green");
      }
    }
    
    setListColor(pos, color){
      var element = document.getElementById('el_' + (pos).toString());
      element.classList.add(color);
    }

    setDefaultListColor(){
      for(let i = 1; i<7; i++){
        var element = document.getElementById('el_' + (i).toString());
    ...
    setDescription(pos){
      ...
      let bird = birdsData[songBird.stage];
      description.innerText = (bird.find(obj => obj.id === pos)).description;
    }
    
    setSpecies(pos){
      var species = document.getElementById('species');
      let bird = birdsData[songBird.stage];
      species.innerText = (bird.find(obj => obj.id === pos)).species;
    }
    
    setName(pos){
        var name = document.getElementById('name');
        let bird = birdsData[songBird.stage];
        name.innerText = (bird.find(obj => obj.id === pos)).name;
    }
    
    setPicture(pos){
        var picture = document.getElementById('picture');
        let bird = birdsData[songBird.stage];
        picture.src = (bird.find(obj => obj.id === pos)).image;
    }
}

document.addEventListener('readystatechange', event => {
    
    songBird = new SongBird();
    songBird.init();
    songBird.getRandom(1,6);
    songBird.setAudio(songBird.rand, 'audio-main', 'audioSource-main');
...
button.onclick = function(e) {
  if(songBird.stage < 5){
    ...
    songBird.getRandom(1,6);
    songBird.setAudio(songBird.rand, 'audio-main', 'audioSource-main');
    songBird.freeze = false;
    var picture = document.getElementById('bird-picture');
    picture.src = "https://birds-quiz.netlify.app/static/media/bird.06a46938.jpg";
    var name = document.getElementById('bird-name');
    name.innerText = "******";
    
    var emptyDiv = document.querySelector("div.center-bottom-right-empty");
    emptyDiv.classList.remove("no-display");
    var div = document.querySelector("div.center-bottom-right");
    div.classList.add("no-display");
    songBird.isClicked = [1,1,1,1,1,1];
    
    document.getElementById('button').setAttribute('disabled', true);

    songBird.setHeaderColor();
    songBird.setDefaultListColor();
    songBird.setAudio(0, 'audio-main', 'audioSource-main');
    songBird.setAudio(0, 'audio', 'audioSource');
  } else{
    var resultDiv = document.querySelector("div.result-page");
    resultDiv.classList.remove("no-display");
    var div = document.querySelector("div.page");
    div.classList.add("no-display");

    var name = document.getElementById('quiz-final');
    name.innerText = "Вы прошли викторину и набрали " + songBird.score + " из 30 возможных баллов";
...
el_1.onclick = function(e) {
    if (songBird.isClicked[0] !== 0){
      songBird.isClicked[0] = 0;
    }

    songBird.getBeep(1);

    songBird.displayChange();
    songBird.setAudio(1, 'audio', 'audioSource');
    songBird.setPicture(1);
    songBird.setName(1);
    songBird.setSpecies(1);
    songBird.setDescription(1);

    songBird.checkResult(1);

    songBird.setListColor(1);
};
el_2.onclick = function(e) {
  if (songBird.isClicked[1] !== 0){
    songBird.isClicked[1] = 0;
  }

  songBird.getBeep(2);

    songBird.displayChange();
    songBird.setAudio(2, 'audio', 'audioSource');
    songBird.setPicture(2);
    songBird.setName(2);
    songBird.setSpecies(2);
    songBird.setDescription(2);

    songBird.checkResult(2);


}
el_3.onclick = function(e) {
  if (songBird.isClicked[2] !== 0){
    songBird.isClicked[2] = 0;
  }

  songBird.getBeep(3);

    songBird.displayChange();
    songBird.setAudio(3, 'audio', 'audioSource');
    songBird.setPicture(3);
    songBird.setName(3);
    songBird.setSpecies(3);
    songBird.setDescription(3);

    songBird.checkResult(3);
}
el_4.onclick = function(e) {
  if (songBird.isClicked[3] !== 0){
    songBird.isClicked[3] = 0;
  }

  songBird.getBeep(4);

    songBird.displayChange();
    songBird.setAudio(4, 'audio', 'audioSource');
    songBird.setPicture(4);
    songBird.setName(4);
    songBird.setSpecies(4);
    songBird.setDescription(4);

    songBird.checkResult(4);
}
el_5.onclick = function(e) {
  if (songBird.isClicked[4] !== 0){
    songBird.isClicked[4] = 0;
  }

  songBird.getBeep(5);

    songBird.displayChange();
    songBird.setAudio(5, 'audio', 'audioSource');
    songBird.setPicture(5);
    songBird.setName(5);
    songBird.setSpecies(5);
    songBird.setDescription(5);

    songBird.checkResult(5);
}
el_6.onclick = function(e) {
  if (songBird.isClicked[5] !== 0){
    songBird.isClicked[5] = 0;
  }

  songBird.getBeep(6);

    songBird.displayChange();
    songBird.setAudio(6, 'audio', 'audioSource');
    songBird.setPicture(6);
    songBird.setName(6);
    songBird.setSpecies(6);
    songBird.setDescription(6);

    songBird.checkResult(6);
}
